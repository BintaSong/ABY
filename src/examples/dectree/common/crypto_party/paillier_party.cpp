...
bool PaillierParty::encrypt(vector<uint64_t> &plaintexts, BYTE* &ciphertexts){
	struct timespec start, end;//add by jianli
	...
	for(int i = 0;i < plaintexts.size();i++){
		...
		clock_gettime(CLOCK_MONOTONIC, &start);//add by jianli
		paillier_dec(&tmp2, m_localpub, m_prv, &res);
		clock_gettime(CLOCK_MONOTONIC, &end);//add by jianli
		printf("CLIENT: decrypt one feature in %.0lf ms \n", getMillies(start, end));//add by jianli

		gmp_printf ("feature%d (%Zd) decryption is (%ZX)\n", i, tmp.m, tmp2.m);//de-comment by jianli
	}
...
void PaillierParty::encSndRcvVec(e_role role, vector<uint64_t> &plaintexts, mpz_t* &ciphertexts, channel* &chan){
	...
	if (role == CLIENT) {
		...
		printf("CLIENT: Encrypted %ld feature vector in %.0lf ms \n\n", plaintexts.size(), getMillies(start, end));
	...
	if (role == SERVER) {
		...
		for (int i = 0; i < n_batch; i++)
		{
			for (int j = 0; j < batch_size; j++)
			{
				...
				mpz_ui_pow_ui(tmp, 2, 104*j);//104=statisticalParam(40)+64
