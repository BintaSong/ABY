...
{
	mpz_mul(pub->n_squared, pub->n, pub->n);//N =n^2
	mpz_add_ui(pub->n_plusone, pub->n, 1);//g= n+1
}

void
complete_prvkey( paillier_prvkey_t* prv, paillier_pubkey_t* pub )
{
	mpz_powm(prv->x, pub->n_plusone, prv->lambda, pub->n_squared);//g^(lamda)
								 ...
								 paillier_get_rand_t get_rand )
{
	...
	*prv = (paillier_prvkey_t*) malloc(sizeof(paillier_prvkey_t));
	...
	complete_prvkey(*prv, *pub);//compute another pri prameter
