...
typedef struct ss_tuple {
	...
	ss_tuple(matrix_z& plaintext, matrix_z sharetext[2]);
	ss_tuple(matrix_z &plaintext, int nrow, int ncol);
...
//node tuples used for convenient prototyping
typedef struct node_tuple {
	node_tuple();
    node_tuple(const node_tuple& s);
	node_tuple& operator= (const node_tuple& rhs);

	node_tuple(int nrow, int ncol);
	node_tuple(matrix_z& plaintext);
	node_tuple(matrix_z &plaintext, int nrow, int ncol);
