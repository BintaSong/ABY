...
/**
 * encrypt values in the plaintext vector and ciphertext in mpz_t
 */
bool PaillierParty::encrypt_Tree(vector<uint64_t> &plaintexts, mpz_t* &ciphertexts){
...
void PaillierParty::encSndRcvVec_Tree(e_role role, vector<uint64_t> &plaintexts, mpz_t* &ciphertexts, channel* &chan){
	std::cout << "plaintexts.size is " << plaintexts.size() << std::endl;
	struct timespec start, end;
	//uint32_t shareBytes =  8;
	uint32_t offset = 0;

	mpz_t tmp, packed_ct;
	mpz_inits(tmp, packed_ct, NULL);

	BYTE *tmp_encSndBuf, *tmp_encRcvBuf;
	tmp_encSndBuf = (BYTE*) calloc(plaintexts.size() * m_nBuflen, 1);
	tmp_encRcvBuf = (BYTE*) calloc(plaintexts.size() * m_nBuflen, 1);

	uint64_t n_batch, batch_size = 15;
	std::cout << "there are " << plaintexts.size()/15 << " encoded nodes" << std::endl;

	if (batch_size > plaintexts.size())
		n_batch = 1;
	else if (plaintexts.size() % batch_size == 0)
		n_batch = plaintexts.size() / batch_size;
	else
		n_batch= plaintexts.size() / batch_size + 1;
	...
	if (role == SERVER) {
		// encrypt element in vector one by one
		clock_gettime(CLOCK_MONOTONIC, &start);
		encrypt(plaintexts, tmp_encSndBuf);
		clock_gettime(CLOCK_MONOTONIC, &end);
		printf("SERVER: Encrypted %ld vectors in %.0lf ms \n\n", plaintexts.size(), getMillies(start, end));

		mpz_t* cts = (mpz_t*) calloc(plaintexts.size(), sizeof(mpz_t));

		for(int i = 0; i < plaintexts.size(); i++){

			mpz_init(cts[i]);
			mpz_import(cts[i], m_nBuflen, 1, 1, 1, 0, tmp_encSndBuf + i * m_nBuflen);
			// gmp_printf ("ciphertext %d is %Zx\n", i, cts[i]);
		}

		// pack three nodes (every 15 elements in vector) as one ciphertext
		for (int i = 0; i < n_batch; i++)
		{
			for (int j = 0; j < batch_size; j++)
			{
				if (j == 0) {
					mpz_set(packed_ct, cts[counter++]);
					continue;
				}
				mpz_ui_pow_ui(tmp, 2, 64*j);//an element has 64 bits
				mpz_powm(tmp, cts[counter++], tmp, m_remotepub->n_squared); //shifting
				mpz_mul(packed_ct, packed_ct, tmp); // adding to the packed CT
				mpz_mod(packed_ct, packed_ct, m_remotepub->n_squared);
				if (counter == plaintexts.size())
					break;
			}
		...
		std::cout << "client received " << c << "messages" << std::endl;
		clock_gettime(CLOCK_MONOTONIC, &end);
		printf("CLIENT: Received encrypted tree in %.0lf ms \n", getMillies(start, end)); 

		ciphertexts = (mpz_t*) calloc(n_batch, sizeof(mpz_t));
