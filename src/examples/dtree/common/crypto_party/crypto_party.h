...
public:
    ...
    virtual bool encrypt_Tree(vector<uint64_t> &plaintexts, mpz_t* &ciphertexts) = 0;

	virtual void encSndRcvVec(e_role role, vector<uint64_t> &plaintexts, mpz_t* &ciphertexts, channel* &chan) = 0;

	virtual void encSndRcvVec_Tree(e_role role, vector<uint64_t> &plaintexts, mpz_t* &ciphertexts, channel* &chan) = 0;
