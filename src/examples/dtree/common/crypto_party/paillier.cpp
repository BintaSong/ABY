							...
							paillier_get_rand_t get_rand )
{
	...
	mpz_init(x);
	mpz_powm(res->c, pub->n_plusone, pt->m, pub->n_squared);//g^m
	mpz_powm(x, r, pub->n, pub->n_squared);//r^n

	mpz_mul(res->c, res->c, x);//g^m * r^n
	mpz_mod(res->c, res->c, pub->n_squared);//(g^m * r^n) mod n^2
