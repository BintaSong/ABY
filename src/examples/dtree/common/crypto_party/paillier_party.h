...
public:
	...
	//add by Jianli
    bool encrypt_Tree(vector<uint64_t> &plaintexts, mpz_t* &ciphertexts);

	void encSndRcvVec(e_role role, vector<uint64_t> &plaintexts, mpz_t* &ciphertexts, channel* &chan);

	//add by Jianli
	void encSndRcvVec_Tree(e_role role, vector<uint64_t> &plaintexts, mpz_t* &ciphertexts, channel* &chan);
