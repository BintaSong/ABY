...
void aes_circuit(e_role role, uint32_t nvals, e_sharing sharing,
  ...
  BYTE sharedIndex[16];
  int2AES16hex(index, sharedIndex);
  for (int i = 0; i < 16; i++) {
    input.SetByte(i, sharedIndex[i]);
  }
  ...
  if (sharing == S_YAO_REV) {
    // Currently the expand_in_sfe and client_only features are not supported in
    // S_YAO_REV
    ...
    s_in_yao = yao_circ->PutSIMDINGate(nyao_circs, input.GetArr(), aes_key_bits,
                                       CLIENT);
    if (nyao_rev_circs > 0)
      s_in_yao_rev = yao_rev_circ->PutSIMDINGate(
          nyao_rev_circs, input.GetArr() + nyao_circs * AES_BYTES, aes_key_bits,
          SERVER);
  ...
  for (int i = 0; i < 16; i++) {
    ...
    // std::cout << std::setw(2) << std::setfill('0') << (std::hex) << (unsigned
    // int)out.GetArr()[i];
  }
  // std::cout << std::endl;
...
void int2AES16hex(uint64_t num, BYTE output[16]){
