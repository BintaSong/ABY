...
void initializeClient(Fss* f, uint32_t numBits, uint32_t numParties) {
    ...
    mpz_class p;
    mpz_ui_pow_ui(p.get_mpz_t(), 2, 32); // p=2^32
    mpz_nextprime(f->prime.get_mpz_t(), p.get_mpz_t()); // Set f->prime.get_mpz_t() 
    // to the next prime greater than p.get_mpz_t().
...
void generateTreeEq(Fss* f, ServerKeyEq* k0, ServerKeyEq* k1, uint64_t a_i, uint64_t b_i){
    uint32_t n = f->numBits; // 在Initialize client中, use 10 bits in domain as example
    ...
    int a = getBit(a_i, (64-n+1)); // 得到一个最后一位为a_i的第(n-1)位,其余位为0的一个数
    int na = a ^ 1; // ^ is XOR operator
